[Mesh]
  file = out_PbTiO3_bulk_len-9_icConst111_n3500_0field_1wall_restart_DIRECT.e
  uniform_refine=0
  distribution=serial
[]
...
#active='potential_int potential_ext'
  [./polar_x]
    ...
    initial_from_file_timestep = 500
    block='2'
  [../]
  [./polar_y]
    #scaling=1e14
...
[Executioner]
  ...
  dt=5e-14   #adjustable within a few orders of magnitude. It seems that we need small dt steps but large num_steps.
  dtmin=1e-15
  #l_max_its=1200
#NOTE: First time step calculates the depolarization field due to the unphysical initial condition. Energy may increase, which is allowed.
  ...
  petsc_options_iname='-ksp_type  -pc_type -pc_factor_zeropivot' 
  petsc_options_value='gmres        lu                1e-50' 
...
print_perf_log = true
  ...
  [./out]
    type = Exodus
    file_base = out_PbTiO3_bulk_len-9_icConst111_n3500_0field_1wall_restart_DIRECT_LU
    output_initial = true
