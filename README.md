...
Ferret still exists under the open-source GNU License but to access the repository you will need either developer access or an RSA deploy key registered with the current institute at which you are employed. We freely provide these keys in order to track repository usage and measure impact. We can also allow write-access on request.
