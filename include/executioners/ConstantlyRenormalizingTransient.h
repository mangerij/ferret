...
class ConstantlyRenormalizingTransient;
class TimeStepper;
class FEProblemBase;
class Kernel;

template <>
InputParameters validParams<ConstantlyRenormalizingTransient>();

/**
 * Transient executioners usually loop through a number of timesteps... calling solve()
 * for each timestep.
 */
class ConstantlyRenormalizingTransient : public Executioner
{
public:
  /**
   * Constructor
   ...
   */
  ConstantlyRenormalizingTransient(const InputParameters & parameters);
