...
struct FInterpMatrixKernelR : FInterpAbstractMatrixKernel<FReal>
{
  ...
  static const char* getID() { return "ONE_OVER_R"; }
                                    ...
                                    ValueClass block[1], ValueClass blockDerivative[3]) const
    {
        ...
        const ValueClass diffy = (y1-y2);
        const ValueClass diffz = (z1-z2);
        const ValueClass r2 = diffx*diffx + diffy*diffy + diffz*diffz; // Xikai
        const ValueClass one_over_r = FMath::One<ValueClass>() / FMath::Sqrt(r2); // Xikai

        const ValueClass one_over_r3 = one_over_r*one_over_r*one_over_r;

        const ValueClass zero = FMath::Zero<ValueClass>();

        if ( FMath::ConvertTo<double,ValueClass>(r2) < 1e-5) { // avoid singularity, Xikai
          block[0] = zero;
          blockDerivative[0] = zero;
          blockDerivative[1] = zero;
          blockDerivative[2] = zero;
        } else {
          block[0] = one_over_r;
...
struct FInterpMatrixKernelRx2 : FInterpAbstractMatrixKernel<FReal>
{
  ...
  static const char* getID() { return "ONE_OVER_R_CUBIC_DX"; }
                   ...
                   const ValueClass& x2, const ValueClass& y2, const ValueClass& z2) const
    {
        ...
        if ( FMath::ConvertTo<double,ValueClass>(r2) < 1e-5 ) { return FMath::ConvertTo<ValueClass,double>(0.0); }
        else {
                                    ...
                                    ValueClass block[1], ValueClass blockDerivative[3]) const
    {
        ...
        if ( FMath::ConvertTo<double,ValueClass>(r2) < 1e-5 ) { block[0] = FMath::ConvertTo<ValueClass,double>(0.0); }
        else {
          const ValueClass one_over_r = FMath::One<ValueClass>() / FMath::Sqrt(r2);
          const ValueClass one_over_r3 = one_over_r*one_over_r*one_over_r;
