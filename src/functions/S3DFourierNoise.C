...
// note small adjustments made to this file for Ferret purposes
...
registerMooseObject("FerretApp", S3DFourierNoise);

template <>
InputParameters
validParams<S3DFourierNoise>()
{
...
S3DFourierNoise::S3DFourierNoise(const InputParameters & parameters)
  : Function(parameters),
    ...
    _fe_problem(*getCheckedPointerParam<FEProblemBase *>("_fe_problem_base"))
{
  ...
  if (isParamValid("num_terms"))
  {
    ...
    const Real scale = 2.0 * (2.0 * libMesh::pi) / _lambda; //3 instead of 2?
    ...
    for (auto & f : _series)
    {
      ...
      do
      {
        ...
        const Real z = rng.rand(0) - 0.5;

        f.k = RealVectorValue(x, y, z);
        r2 = f.k.norm_sq();
      } while (r2 > 0.25);
    ...
    for (int x = 0; x < xmax; ++x)
      for (int y = -ymax; y < ymax; ++y)
        for (int z = -zmax; z < zmax; ++z)
          ...
          {
            SeriesItem f;
            f.k = RealVectorValue(x * dx, y * dy, z * dz);
...
Real
S3DFourierNoise::value(Real, const Point & p) const
{
  Real v = 0.0;
