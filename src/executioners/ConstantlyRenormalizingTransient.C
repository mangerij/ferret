/*
  ...
  //TODO: this is a work in progress file - it is copied from MOOSE framework under fair use

**/

#include "ConstantlyRenormalizingTransient.h"
...
registerMooseObject("MooseApp", ConstantlyRenormalizingTransient);

template <>
InputParameters
validParams<ConstantlyRenormalizingTransient>()
{
...
ConstantlyRenormalizingTransient::ConstantlyRenormalizingTransient(const InputParameters & parameters)
...
void
ConstantlyRenormalizingTransient::init()
{
  if (!_time_stepper.get())
  {
    ...
    pars.set<ConstantlyRenormalizingTransient *>("_executioner") = this;
...
void
ConstantlyRenormalizingTransient::preExecute()
{
...
void
ConstantlyRenormalizingTransient::preStep()
{
  _time_stepper->preStep();
}

void
ConstantlyRenormalizingTransient::postStep()
{
  _time_stepper->postStep();
}

void
ConstantlyRenormalizingTransient::execute()
{
  preExecute();
...
void
ConstantlyRenormalizingTransient::computeDT()
{
  _time_stepper->computeStep(); // This is actually when DT gets computed
}

void
ConstantlyRenormalizingTransient::incrementStepOrReject()
{
  if (lastSolveConverged())
  {
...
void
ConstantlyRenormalizingTransient::takeStep(Real input_dt)
{
  _dt_old = _dt;
...
void
ConstantlyRenormalizingTransient::endStep(Real input_time)
{
  if (input_time == -1.0)
    _time = _time_old + _dt;
  else
...
Real
ConstantlyRenormalizingTransient::computeConstrainedDT()
{
...
Real
ConstantlyRenormalizingTransient::getDT()
{
  return _time_stepper->getCurrentDT();
}

bool
ConstantlyRenormalizingTransient::keepGoing()
{
...
void
ConstantlyRenormalizingTransient::estimateTimeError()
{
}

bool
ConstantlyRenormalizingTransient::lastSolveConverged() const
{
  return _last_solve_converged;
}

void
ConstantlyRenormalizingTransient::postExecute()
{
  _time_stepper->postExecute();
}

void
ConstantlyRenormalizingTransient::setTargetTime(Real target_time)
{
  _target_time = target_time;
}

Real
ConstantlyRenormalizingTransient::getSolutionChangeNorm()
{
  return _solution_change_norm;
}

void
ConstantlyRenormalizingTransient::setupTimeIntegrator()
{
...
std::string
ConstantlyRenormalizingTransient::getTimeStepperName()
{
  if (_time_stepper)
  {
...
Real
ConstantlyRenormalizingTransient::relativeSolutionDifferenceNorm()
{
