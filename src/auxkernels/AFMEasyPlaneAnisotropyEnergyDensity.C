...
registerMooseObject("FerretApp", AFMEasyPlaneAnisotropyEnergyDensity);

InputParameters AFMEasyPlaneAnisotropyEnergyDensity::validParams()
{

  InputParameters params = AuxKernel::validParams();
  params.addClassDescription("Calculates the free energy density due easy-plane (or easy-axis) anisotropy");
...
AFMEasyPlaneAnisotropyEnergyDensity::AFMEasyPlaneAnisotropyEnergyDensity(const InputParameters & parameters) :
...
Real
AFMEasyPlaneAnisotropyEnergyDensity::computeValue()
{
  RealVectorValue w(_polar_x[_qp], _polar_y[_qp], _polar_z[_qp]);
  RealVectorValue f = w/std::sqrt(w*w);
  return _energy_scale*(-(_K1[_qp]*Utility::pow<2>(_mag_x[_qp]*f(0) + _mag_y[_qp]*f(1) + _mag_z[_qp]*f(2))));
}
