...
InputParameters validParams<ElectrostaticEnergyDensity>()
{
  ...
  params.addRequiredCoupledVar("potential", "The total electrostatic potential");
...
ElectrostaticEnergyDensity::computeValue()
{
  ...
  return 0.5*(P*_potential_grad[_qp])*pow(_len_scale,2.0)+0.5*(P*_potential_ext_grad[_qp])*pow(_len_scale,2.0);
}
