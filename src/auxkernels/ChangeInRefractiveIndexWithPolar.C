...
InputParameters ChangeInRefractiveIndexWithPolar::validParams()
...
ChangeInRefractiveIndexWithPolar::computeValue()
{
  ...
  return - 0.5 * Utility::pow<3>(1.0 / std::pow(_indicatrix[_qp](_index_i, _index_j), 0.5)) *  (_delta_indicatrix[_qp](_index_k, _index_l) + _delta_PO_tensor[_qp](_index_k, _index_l));
}
