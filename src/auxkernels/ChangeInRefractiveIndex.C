...
InputParameters ChangeInRefractiveIndex::validParams()

{
  InputParameters params = AuxKernel::validParams();
  params.addClassDescription("Calculates the changes to local refractive index.");
...
ChangeInRefractiveIndex::ChangeInRefractiveIndex(const InputParameters & parameters) :
  AuxKernel(parameters),
   ...
   _delta_indicatrix(getMaterialProperty<RankTwoTensor>("delta_indicatrix")) //or need RealTensorValue
{
}
...
ChangeInRefractiveIndex::computeValue()
{
  ...
  return - 0.5 * Utility::pow<3>(1.0 / std::pow(_indicatrix[_qp](_index_i, _index_j), 0.5)) *  _delta_indicatrix[_qp](_index_k, _index_l);
}
