...
#include "AFMSingleIonAnisotropyAltEnergy.h"

registerMooseObject("FerretApp", AFMSingleIonAnisotropyAltEnergy);

InputParameters AFMSingleIonAnisotropyAltEnergy::validParams()
{
...
AFMSingleIonAnisotropyAltEnergy::AFMSingleIonAnisotropyAltEnergy(const InputParameters & parameters) :
...
Real
AFMSingleIonAnisotropyAltEnergy::computeQpIntegral()
{
  return _energy_scale*(_K1c[_qp]*Utility::pow<2>(_mag1_x[_qp])*Utility::pow<2>(_mag1_y[_qp]) + _K2c[_qp]*Utility::pow<2>(_mag1_x[_qp])*Utility::pow<2>(_mag1_y[_qp])*Utility::pow<2>(_mag1_z[_qp]) + _K1c[_qp]*(Utility::pow<2>(_mag1_x[_qp]) + Utility::pow<2>(_mag1_y[_qp]))*Utility::pow<2>(_mag1_z[_qp]) + _K1c[_qp]*Utility::pow<2>(_mag2_x[_qp])*Utility::pow<2>(_mag2_y[_qp]) + _K2c[_qp]*Utility::pow<2>(_mag2_x[_qp])*Utility::pow<2>(_mag2_y[_qp])*Utility::pow<2>(_mag2_z[_qp]) + _K1c[_qp]*(Utility::pow<2>(_mag2_x[_qp]) + Utility::pow<2>(_mag2_y[_qp]))*Utility::pow<2>(_mag2_z[_qp]));
}
