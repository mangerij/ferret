...
defineLegacyParams(MultiAppFieldAddTransfer);

InputParameters
MultiAppFieldAddTransfer::validParams()
{
...
MultiAppFieldAddTransfer::MultiAppFieldAddTransfer(const InputParameters & parameters)
  : MultiAppTransfer(parameters)
{
}

void
MultiAppFieldAddTransfer::initialSetup()
{
...
void
MultiAppFieldAddTransfer::transferDofObject(libMesh::DofObject * to_object,
  ...
  for (unsigned int vc = 0; vc < to_var.count(); ++vc)
    if (to_object->n_dofs(to_var.sys().number(), to_var.number() + vc) >
           ...
           ++comp)
      {
        ...
        to_solution.add(dof, from_value);
      }
}

void
MultiAppFieldAddTransfer::transfer(FEProblemBase & to_problem, FEProblemBase & from_problem)
{
