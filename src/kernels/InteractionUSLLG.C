...
InteractionUSLLG::computeQpResidual()
{
  if (_component == 0)
  {
    return (_g0*_test[_i][_qp]*std::sin(_polar_theta[_qp])*(-(_potential_H_int_grad[_qp](1)*_Ms*std::cos(_azimuth_phi[_qp])) + _potential_H_int_grad[_qp](0)*_Ms*std::sin(_azimuth_phi[_qp]) - _alpha*_Ms*std::cos(_polar_theta[_qp])*(_potential_H_int_grad[_qp](0)*std::cos(_azimuth_phi[_qp]) + _potential_H_int_grad[_qp](1)*std::sin(_azimuth_phi[_qp])) + _alpha*_potential_H_int_grad[_qp](2)*_Ms*std::sin(_polar_theta[_qp])))/((1 + Utility::pow<2>(_alpha))*_Ms*std::sqrt(Utility::pow<2>(std::sin(_polar_theta[_qp]))));
  }
  else if (_component == 1)
  {
    return (_g0*_test[_i][_qp]*(-(_potential_H_int_grad[_qp](2)*_Ms*(1.0/std::sin(_polar_theta[_qp]))) + _alpha*_Ms*Utility::pow<2>((1.0/std::sin(_polar_theta[_qp])))*(-(_potential_H_int_grad[_qp](1)*std::cos(_azimuth_phi[_qp])) + _potential_H_int_grad[_qp](0)*std::sin(_azimuth_phi[_qp])) - (std::cos(_polar_theta[_qp])/std::sin(_polar_theta[_qp]))*(1.0/std::sin(_polar_theta[_qp]))*(_potential_H_int_grad[_qp](0)*_Ms*std::cos(_azimuth_phi[_qp]) - _potential_H_int_grad[_qp](1)*_Ms*std::sin(_azimuth_phi[_qp])))*std::sin(_polar_theta[_qp]))/((1 + Utility::pow<2>(_alpha))*_Ms);
  }
  else
    return 0.0;
}
