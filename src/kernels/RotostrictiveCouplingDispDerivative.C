...
RotostrictiveCouplingDispDerivative::computeQpResidual()
{
  if (_component == 0)
  {
    return (2*_antiferrodis_A_x[_qp]*_antiferrodis_A_y[_qp]*_C44*_grad_test[_i][_qp](1)*_R44 + 2*_antiferrodis_A_x[_qp]*_antiferrodis_A_z[_qp]*_C44*_grad_test[_i][_qp](2)*_R44 + _grad_test[_i][_qp](0)*(_C12*(std::pow(_antiferrodis_A_z[_qp],2)*_R11 + (std::pow(_antiferrodis_A_x[_qp],2) + std::pow(_antiferrodis_A_y[_qp],2))*_R12) + _C12*(std::pow(_antiferrodis_A_y[_qp],2)*_R11 + (std::pow(_antiferrodis_A_x[_qp],2) + std::pow(_antiferrodis_A_z[_qp],2))*_R12) + 
      _C11*((std::pow(_antiferrodis_A_y[_qp],2) + std::pow(_antiferrodis_A_z[_qp],2))*_R12 + std::pow(_antiferrodis_A_x[_qp],2)*_R44)));
  }
  else if (_component == 1)
  {
    return (2*_antiferrodis_A_x[_qp]*_antiferrodis_A_y[_qp]*_C44*_grad_test[_i][_qp](0)*_R44 + 2*_antiferrodis_A_y[_qp]*_antiferrodis_A_z[_qp]*_C44*_grad_test[_i][_qp](2)*_R44 + _grad_test[_i][_qp](1)*(_C12*(std::pow(_antiferrodis_A_z[_qp],2)*_R11 + (std::pow(_antiferrodis_A_x[_qp],2) + std::pow(_antiferrodis_A_y[_qp],2))*_R12) + _C11*(std::pow(_antiferrodis_A_y[_qp],2)*_R11 + (std::pow(_antiferrodis_A_x[_qp],2) + std::pow(_antiferrodis_A_z[_qp],2))*_R12) + 
      _C12*((std::pow(_antiferrodis_A_y[_qp],2) + std::pow(_antiferrodis_A_z[_qp],2))*_R12 + std::pow(_antiferrodis_A_x[_qp],2)*_R44)));
  }
  else if (_component == 2)
  {
    return (2*_antiferrodis_A_x[_qp]*_antiferrodis_A_z[_qp]*_C44*_grad_test[_i][_qp](0)*_R44 + 2*_antiferrodis_A_y[_qp]*_antiferrodis_A_z[_qp]*_C44*_grad_test[_i][_qp](1)*_R44 + _grad_test[_i][_qp](2)*(_C11*(std::pow(_antiferrodis_A_z[_qp],2)*_R11 + (std::pow(_antiferrodis_A_x[_qp],2) + std::pow(_antiferrodis_A_y[_qp],2))*_R12) + _C12*(std::pow(_antiferrodis_A_y[_qp],2)*_R11 + (std::pow(_antiferrodis_A_x[_qp],2) + std::pow(_antiferrodis_A_z[_qp],2))*_R12) + 
      _C12*((std::pow(_antiferrodis_A_y[_qp],2) + std::pow(_antiferrodis_A_z[_qp],2))*_R12 + std::pow(_antiferrodis_A_x[_qp],2)*_R44)));
  }
  else
    return 0.0;
...
RotostrictiveCouplingDispDerivative::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (_component == 0)
  {
    ...
    {
      return _phi[_j][_qp] * (2*_antiferrodis_A_y[_qp]*_C44*_grad_test[_i][_qp](1)*_R44 + 2*_antiferrodis_A_z[_qp]*_C44*_grad_test[_i][_qp](2)*_R44 + _grad_test[_i][_qp](0)*(4*_antiferrodis_A_x[_qp]*_C12*_R12 + 2*_antiferrodis_A_x[_qp]*_C11*_R44));
    }
  ...
  else if (_component == 1)
  {
    if (jvar == _antiferrodis_A_x_var)
    {
      return _phi[_j][_qp] * (2*_antiferrodis_A_y[_qp]*_C44*_grad_test[_i][_qp](0)*_R44 + _grad_test[_i][_qp](1)*(2*_antiferrodis_A_x[_qp]*_C11*_R12 + 2*_antiferrodis_A_x[_qp]*_C12*_R12 + 2*_antiferrodis_A_x[_qp]*_C12*_R44));
    }
    else if (jvar == _antiferrodis_A_y_var)
    {
  ...
  else if (_component == 2)
  {
    if (jvar == _antiferrodis_A_x_var)
    {
      return _phi[_j][_qp] * (2*_antiferrodis_A_z[_qp]*_C44*_grad_test[_i][_qp](0)*_R44 + _grad_test[_i][_qp](2)*(2*_antiferrodis_A_x[_qp]*_C11*_R12 + 2*_antiferrodis_A_x[_qp]*_C12*_R12 + 2*_antiferrodis_A_x[_qp]*_C12*_R44));
    }
    else if (jvar == _antiferrodis_A_y_var)
    {
