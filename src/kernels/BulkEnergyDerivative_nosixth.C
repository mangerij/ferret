/**
 * @file   BulkEnergyDerivative_nosixth.C
...
//Constructor
BulkEnergyDerivative_nosixth::BulkEnergyDerivative_nosixth(const std::string & name, InputParameters parameters)
...
Real
BulkEnergyDerivative_nosixth::computeQpResidual()
{
  ...
  return ((2*_alpha1*_polar_i[_qp]+
	  ...
	  2*_alpha12*_polar_i[_qp]*(pow(_polar_j[_qp],2)+pow(_polar_k[_qp],2)))*_test[_i][_qp])*pow(_len_scale,3.0)*_energy_scale;
}

Real
BulkEnergyDerivative_nosixth::computeQpJacobian()
{
  const VariableValue& _polar_i= (_component==0)? _polar_x : (_component==1)? _polar_y: _polar_z;
  const VariableValue& _polar_j= (_component==0)? _polar_y : (_component==1)? _polar_z: _polar_x;
  const VariableValue& _polar_k= (_component==0)? _polar_z : (_component==1)? _polar_x: _polar_y;
   return (2*_alpha1+12*_alpha11*pow(_polar_i[_qp],2)+
	  2*_alpha12*(pow(_polar_j[_qp],2)+pow(_polar_k[_qp],2)))*_test[_i][_qp]*_phi[_j][_qp]*pow(_len_scale,3.0)*_energy_scale;
}

Real
BulkEnergyDerivative_nosixth::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real r;
  ...
  if(jvar==_polar_x_var || jvar==_polar_y_var || jvar==_polar_z_var){
    ...
    return r*_test[_i][_qp]*_phi[_j][_qp]*pow(_len_scale,3.0)*_energy_scale;
  }else
    return 0.0;
}
