/**
 * @file   BulkEnergyDerivativeFourth.C
...
template<>
InputParameters validParams<BulkEnergyDerivativeFourth>()
{
...
//Constructor
BulkEnergyDerivativeFourth::BulkEnergyDerivativeFourth(const std::string & name, InputParameters parameters)
...
Real
BulkEnergyDerivativeFourth::computeQpResidual()
{
...
Real
BulkEnergyDerivativeFourth::computeQpJacobian()
{
  const VariableValue& _polar_i= (_component == 0)? _polar_x : (_component == 1)? _polar_y: _polar_z;
  const VariableValue& _polar_j= (_component == 0)? _polar_y : (_component == 1)? _polar_z: _polar_x;
  const VariableValue& _polar_k= (_component == 0)? _polar_z : (_component == 1)? _polar_x: _polar_y;

  return (2 * _alpha1 + 12 * _alpha11 * std::pow(_polar_i[_qp], 2) + 2 * _alpha12 * (std::pow(_polar_j[_qp], 2) + std::pow(_polar_k[_qp], 2)) ) * _test[_i][_qp] * _phi[_j][_qp] * std::pow(_len_scale, 3.0);
}

Real
BulkEnergyDerivativeFourth::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real r;
