...
#include "LongitudinalLocalLLB.h"
#include "libmesh/utility.h"

registerMooseObject("FerretApp", LongitudinalLocalLLB);

template<>
InputParameters validParams<LongitudinalLocalLLB>()
{
  ...
  params.addRequiredCoupledVar("alpha_long", "The local LLB damping term");
  return params;
}

LongitudinalLocalLLB::LongitudinalLocalLLB(const InputParameters & parameters)
  :Kernel(parameters),
  ...
  _alpha_long(coupledValue("alpha_long")),
...
Real
LongitudinalLocalLLB::computeQpResidual()
{
...
Real
LongitudinalLocalLLB::computeQpJacobian()
{
  if (_component == 0)
  {
...
Real
LongitudinalLocalLLB::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (_component == 0)
  {
    if (jvar == _mag_y_var)
