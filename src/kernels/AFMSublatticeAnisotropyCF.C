...
#include "AFMSublatticeAnisotropyCF.h"
#include "libmesh/utility.h"

registerMooseObject("FerretApp", AFMSublatticeAnisotropyCF);

InputParameters AFMSublatticeAnisotropyCF::validParams()
{
  InputParameters params = Kernel::validParams();
  params.addClassDescription("Calculates a residual contribution for the magnetic anisotropy energy corresponding to single-ion anistropy (uniaxial in a perpendicular direction to P).");
...
AFMSublatticeAnisotropyCF::AFMSublatticeAnisotropyCF(const InputParameters & parameters)
  :Kernel(parameters),
...
Real
AFMSublatticeAnisotropyCF::computeQpResidual()
{
  if (_component == 0)
  {
    RealVectorValue w(-_polar_y[_qp]*_polar_y[_qp]-_polar_z[_qp]*_polar_z[_qp], _polar_x[_qp]*_polar_y[_qp], _polar_x[_qp]*_polar_z[_qp]);
    RealVectorValue f = w/std::sqrt(w*w);
    return -(2.0*_g0[_qp]*_K2[_qp]*(_mag_x[_qp]*f(0) + _mag_y[_qp]*f(1) + _mag_z[_qp]*f(2))*(-(_mag_z[_qp]*f(1)) + _mag_y[_qp]*f(2) + _alpha[_qp]*_Ms[_qp]*(-(Utility::pow<2>(_mag_y[_qp])*f(0)) + _mag_x[_qp]*_mag_y[_qp]*f(1) + _mag_z[_qp]*(-(_mag_z[_qp]*f(0)) + _mag_x[_qp]*f(2))))*_test[_i][_qp])/
...
Real
AFMSublatticeAnisotropyCF::computeQpJacobian()
{
  if (_component == 0)
  {
    RealVectorValue w(-_polar_y[_qp]*_polar_y[_qp]-_polar_z[_qp]*_polar_z[_qp], _polar_x[_qp]*_polar_y[_qp], _polar_x[_qp]*_polar_z[_qp]);
    RealVectorValue f = w/std::sqrt(w*w);
    return -(2.0*_g0[_qp]*_K2[_qp]*(-(_mag_z[_qp]*f(0)*f(1)) + _mag_y[_qp]*f(0)*f(2) + _alpha[_qp]*_Ms[_qp]*(Utility::pow<2>(_mag_y[_qp])*(-Utility::pow<2>(f(0)) + Utility::pow<2>(f(1))) + 2.0*_mag_y[_qp]*f(1)*(_mag_x[_qp]*f(0) + _mag_z[_qp]*f(2)) + 
  ...
  else if (_component == 1)
  {
    RealVectorValue w(-_polar_y[_qp]*_polar_y[_qp]-_polar_z[_qp]*_polar_z[_qp], _polar_x[_qp]*_polar_y[_qp], _polar_x[_qp]*_polar_z[_qp]);
    RealVectorValue f = w/std::sqrt(w*w);
    return -(2.0*_g0[_qp]*_K2[_qp]*(-(_mag_z[_qp]*Utility::pow<2>(f(1))) + _mag_x[_qp]*f(0)*f(2) + 2*_mag_y[_qp]*f(1)*f(2) + _mag_z[_qp]*Utility::pow<2>(f(2)) + 
  ...
  else if (_component == 2)
  {
    RealVectorValue w(-_polar_y[_qp]*_polar_y[_qp]-_polar_z[_qp]*_polar_z[_qp], _polar_x[_qp]*_polar_y[_qp], _polar_x[_qp]*_polar_z[_qp]);
    RealVectorValue f = w/std::sqrt(w*w);
    return -(2.0*_g0[_qp]*_K2[_qp]*(-((2*_alpha[_qp]*_Ms[_qp]*_mag_z[_qp]*f(0) + f(1))*(_mag_x[_qp]*f(0) + _mag_y[_qp]*f(1))) - 2.0*_mag_z[_qp]*f(1)*f(2) + 
...
Real
AFMSublatticeAnisotropyCF::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (_component == 0)
  {
    if (jvar == _mag_y_var)
  ...
  else if (_component == 1)
  {
    ...
    else if (jvar == _mag_z_var)
    {
      RealVectorValue w(-_polar_y[_qp]*_polar_y[_qp]-_polar_z[_qp]*_polar_z[_qp], _polar_x[_qp]*_polar_y[_qp], _polar_x[_qp]*_polar_z[_qp]);
      RealVectorValue f = w/std::sqrt(w*w);
      return -(2.0*_g0[_qp]*_K2[_qp]*(-((2.0*_alpha[_qp]*_Ms[_qp]*_mag_z[_qp]*f(0) + f(1))*(_mag_x[_qp]*f(0) + _mag_y[_qp]*f(1))) - 2*_mag_z[_qp]*f(1)*f(2) + 
  ...
  else if (_component == 2)
  {
    if (jvar == _mag_x_var)
    {
      RealVectorValue w(-_polar_y[_qp]*_polar_y[_qp]-_polar_z[_qp]*_polar_z[_qp], _polar_x[_qp]*_polar_y[_qp], _polar_x[_qp]*_polar_z[_qp]);
      RealVectorValue f = w/std::sqrt(w*w);
      return -(2.0*_g0[_qp]*_K2[_qp]*(-(_mag_z[_qp]*f(0)*f(1)) + _mag_y[_qp]*f(0)*f(2) + _alpha[_qp]*_Ms[_qp]*(Utility::pow<2>(_mag_y[_qp])*(-Utility::pow<2>(f(0)) + Utility::pow<2>(f(1))) + 2*_mag_y[_qp]*f(1)*(_mag_x[_qp]*f(0) + _mag_z[_qp]*f(2)) + 
