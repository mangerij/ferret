...
InputParameters validParams<BulkEnergyDerivativePSTO>()
{
  ...
  params.addRequiredParam<Real>("E_x", "The applied electric field for the x component");
  params.addRequiredParam<Real>("E_y", "The applied electric field for the y component");
...
BulkEnergyDerivativePSTO::BulkEnergyDerivativePSTO(const InputParameters & parameters)
  :Kernel(parameters),
   ...
   _E_x(getParam<Real>("E_x")),
   _E_y(getParam<Real>("E_y")),
...
BulkEnergyDerivativePSTO::computeQpResidual()
{
  ...
  Real Efree = 0.0;
  Efree += (_alpha1 * (_T - _Tc) * (std::pow(_polar_i[_qp], 2.0) + std::pow(_polar_j[_qp], 2.0)) + _alpha2 * (std::pow(_polar_i[_qp], 4.0) + (std::pow(_polar_j[_qp], 4.0)) + _alpha3 * std::pow(_polar_i[_qp], 2.0) * std::pow(_polar_j[_qp], 2.0) + _alpha4 * (std::pow(_polar_i[_qp], 6.0) + std::pow(_polar_j[_qp], 6.0)) + _alpha5 * (std::pow(_polar_i[_qp], 4.0) * std::pow(_polar_j[_qp], 2.0) + std::pow(_polar_i[_qp], 2.0) * std::pow(_polar_j[_qp], 4.0)) + (_x1 * (std::pow(_polar_i[_qp], 2.0) + std::pow(_polar_j[_qp], 2.0)) + _x2 * (std::pow(_polar_i[_qp], 4.0) + std::pow(_polar_j[_qp], 4.0)) + _x3 * std::pow(_polar_i[_qp], 2.0) * std::pow(_polar_j[_qp], 2.0)) * _epsilon + (_x4 * (std::pow(_polar_i[_qp], 2.0) + std::pow(_polar_j[_qp], 2.0)) + _x5 * (std::pow(_polar_i[_qp], 4.0) + std::pow(_polar_j[_qp], 4.0)) + _x6 * std::pow(_polar_i[_qp], 2.0) * std::pow(_polar_j[_qp], 2.0)) * std::pow(_epsilon, 2.0) - _polar_i[_qp] * _E_x - _polar_j[_qp] * _E_y) * _test[_i][_qp]);
