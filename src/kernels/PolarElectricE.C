...
{
  return -0.5*(_polar_x[_qp]*_grad_test[_i][_qp](0)+_polar_y[_qp]*_grad_test[_i][_qp](1)+_polar_z[_qp]*_grad_test[_i][_qp](2))*pow(_len_scale,2.0);
}
...
PolarElectricE::computeQpOffDiagJacobian(unsigned int jvar)
{
  ...
  if (jvar == _polar_x_var)
    ...
    return 0.0;
  return -0.5*_grad_test[_i][_qp](component)*_phi[_j][_qp]*pow(_len_scale,2.0);
}
