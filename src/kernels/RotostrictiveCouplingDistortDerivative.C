...
RotostrictiveCouplingDistortDerivative::computeQpResidual()
{
  if (_component == 0)
  {
    return -0.5*_test[_i][_qp] * (-2*_antiphase_A_x[_qp]*_r11[_qp]*_u_x_grad[_qp](0) - 2*_r44[_qp]*((_antiphase_A_y[_qp]*(_u_x_grad[_qp](1) + _u_y_grad[_qp](0)))/2. + (_antiphase_A_z[_qp]*(_u_x_grad[_qp](2) + _u_z_grad[_qp](0)))/2.) - _r12[_qp]*(2*_antiphase_A_x[_qp]*_u_y_grad[_qp](1) + 2*_antiphase_A_x[_qp]*_u_z_grad[_qp](2)));
  }
  else if (_component == 1)
  {
    return -0.5*_test[_i][_qp] * (-2*_antiphase_A_y[_qp]*_r11[_qp]*_u_y_grad[_qp](1) - 2*_r44[_qp]*((_antiphase_A_x[_qp]*(_u_x_grad[_qp](1) + _u_y_grad[_qp](0)))/2. + (_antiphase_A_z[_qp]*(_u_y_grad[_qp](2) + _u_z_grad[_qp](1)))/2.) - _r12[_qp]*(2*_antiphase_A_y[_qp]*_u_x_grad[_qp](0) + 2*_antiphase_A_y[_qp]*_u_z_grad[_qp](2)));
  }
  else if (_component == 2)
  {
    return -0.5*_test[_i][_qp] * (-(_r12[_qp]*(2*_antiphase_A_z[_qp]*_u_x_grad[_qp](0) + 2*_antiphase_A_z[_qp]*_u_y_grad[_qp](1))) - 2*_r44[_qp]*((_antiphase_A_x[_qp]*(_u_x_grad[_qp](2) + _u_z_grad[_qp](0)))/2. + (_antiphase_A_y[_qp]*(_u_y_grad[_qp](2) + _u_z_grad[_qp](1)))/2.) - 2*_antiphase_A_z[_qp]*_r11[_qp]*_u_z_grad[_qp](2));
  }
  else
    return 0.0;
}
...
RotostrictiveCouplingDistortDerivative::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (_component == 0)
  {
    ...
    {
      return -0.5*_test[_i][_qp] * (-2*_antiphase_A_x[_qp]*_grad_phi[_j][_qp](0)*_r11[_qp] - 2*((_antiphase_A_y[_qp]*_grad_phi[_j][_qp](1))/2. + (_antiphase_A_z[_qp]*_grad_phi[_j][_qp](2))/2.)*_r44[_qp]);
    }
    else if (jvar == _u_y_var)
    {
      return -0.5*_test[_i][_qp] * (-2*_antiphase_A_x[_qp]*_grad_phi[_j][_qp](1)*_r12[_qp] - _antiphase_A_y[_qp]*_grad_phi[_j][_qp](0)*_r44[_qp]);
    }
    else if (jvar == _u_z_var)
    {
      return -0.5*_test[_i][_qp] * (-2*_antiphase_A_x[_qp]*_grad_phi[_j][_qp](2)*_r12[_qp] - _antiphase_A_z[_qp]*_grad_phi[_j][_qp](0)*_r44[_qp]);
    }
    else
    {
      return 0.0;
  ...
  else if (_component == 1)
  {
    ...
    {
      return -0.5*_test[_i][_qp] * (-2*_antiphase_A_y[_qp]*_grad_phi[_j][_qp](0)*_r12[_qp] - _antiphase_A_x[_qp]*_grad_phi[_j][_qp](1)*_r44[_qp]);
    }
    else if (jvar == _u_y_var)
    {
      return -0.5*_test[_i][_qp] * (-2*_antiphase_A_y[_qp]*_grad_phi[_j][_qp](1)*_r11[_qp] - 2*((_antiphase_A_x[_qp]*_grad_phi[_j][_qp](0))/2. + (_antiphase_A_z[_qp]*_grad_phi[_j][_qp](2))/2.)*_r44[_qp]);
    }
    else if (jvar == _u_z_var)
    {
      return -0.5*_test[_i][_qp] * (-2*_antiphase_A_y[_qp]*_grad_phi[_j][_qp](2)*_r12[_qp] - _antiphase_A_z[_qp]*_grad_phi[_j][_qp](1)*_r44[_qp]);
    }
    else
    {
      return 0.0;
  ...
  else if (_component == 2)
  {
    ...
    {
      return -0.5*_test[_i][_qp] * (-2*_antiphase_A_z[_qp]*_grad_phi[_j][_qp](0)*_r12[_qp] - _antiphase_A_x[_qp]*_grad_phi[_j][_qp](2)*_r44[_qp]);
    }
    else if (jvar == _u_y_var)
    {
      return -0.5*_test[_i][_qp] * (-2*_antiphase_A_z[_qp]*_grad_phi[_j][_qp](1)*_r12[_qp] - _antiphase_A_y[_qp]*_grad_phi[_j][_qp](2)*_r44[_qp]);
    }
    else if (jvar == _u_z_var)
    {
      return -0.5*_test[_i][_qp] * (-2*_antiphase_A_z[_qp]*_grad_phi[_j][_qp](2)*_r11[_qp] - 2*((_antiphase_A_x[_qp]*_grad_phi[_j][_qp](0))/2. + (_antiphase_A_y[_qp]*_grad_phi[_j][_qp](1))/2.)*_r44[_qp]);
    }
    else
    {
      return 0.0;
