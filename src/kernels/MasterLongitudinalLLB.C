...
MasterLongitudinalLLB::computeQpResidual()
{
   ...
   Real _temp=_g0[_qp]*_alpha_long[_qp]*_temp1*(_temp1-_fD)/(1.0+Utility::pow<2>(_alpha[_qp]));
  if (_component == 0)
  {
...
MasterLongitudinalLLB::computeQpJacobian()
{
  if (_component == 0)
  {
  return (_alpha_long[_qp]*_g0[_qp]*(5.0*Utility::pow<4>(_mag_x[_qp]) + 6.0*Utility::pow<2>(_mag_x[_qp])*(Utility::pow<2>(_mag_y[_qp]) + Utility::pow<2>(_mag_z[_qp])) + Utility::pow<2>(Utility::pow<2>(_mag_y[_qp]) + Utility::pow<2>(_mag_z[_qp])) - _fD*(3.0*Utility::pow<2>(_mag_x[_qp]) + Utility::pow<2>(_mag_y[_qp]) + Utility::pow<2>(_mag_z[_qp])))*
     _phi[_j][_qp]*_test[_i][_qp])/(1.0 + Utility::pow<2>(_alpha[_qp]));
  }
  else if (_component == 1)
  {
  return (_alpha_long[_qp]*_g0[_qp]*(Utility::pow<4>(_mag_x[_qp]) + 5.0*Utility::pow<4>(_mag_y[_qp]) + 6.0*Utility::pow<2>(_mag_y[_qp])*Utility::pow<2>(_mag_z[_qp]) + Utility::pow<4>(_mag_z[_qp]) + 2*Utility::pow<2>(_mag_x[_qp])*(3.0*Utility::pow<2>(_mag_y[_qp]) + Utility::pow<2>(_mag_z[_qp])) - 
  ...
  {
  return (_alpha_long[_qp]*_g0[_qp]*(Utility::pow<4>(_mag_x[_qp]) + Utility::pow<4>(_mag_y[_qp]) + 6.0*Utility::pow<2>(_mag_y[_qp])*Utility::pow<2>(_mag_z[_qp]) + 5.0*Utility::pow<4>(_mag_z[_qp]) + 2*Utility::pow<2>(_mag_x[_qp])*(Utility::pow<2>(_mag_y[_qp]) + 3.0*Utility::pow<2>(_mag_z[_qp])) - 
...
MasterLongitudinalLLB::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (_component == 0)
  {
    if (jvar == _mag_y_var)
    {
    return (2.0*_alpha_long[_qp]*_g0[_qp]*_mag_x[_qp]*_mag_y[_qp]*(-_fD + 2*(Utility::pow<2>(_mag_x[_qp]) + Utility::pow<2>(_mag_y[_qp]) + Utility::pow<2>(_mag_z[_qp])))*_phi[_j][_qp]*_test[_i][_qp])/(1.0 + Utility::pow<2>(_alpha[_qp]));
    }
    else if (jvar == _mag_z_var)
    {
    return (2.0*_alpha_long[_qp]*_g0[_qp]*_mag_x[_qp]*_mag_z[_qp]*(-_fD + 2*(Utility::pow<2>(_mag_x[_qp]) + Utility::pow<2>(_mag_y[_qp]) + Utility::pow<2>(_mag_z[_qp])))*_phi[_j][_qp]*_test[_i][_qp])/(1.0 + Utility::pow<2>(_alpha[_qp]));
    }
    else
    {
      return 0.0;
  ...
  else if (_component == 1)
  {
    ...
    {
    return (2.0*_alpha_long[_qp]*_g0[_qp]*_mag_y[_qp]*_mag_z[_qp]*(-_fD + 2*(Utility::pow<2>(_mag_x[_qp]) + Utility::pow<2>(_mag_y[_qp]) + Utility::pow<2>(_mag_z[_qp])))*_phi[_j][_qp]*_test[_i][_qp])/(1.0 + Utility::pow<2>(_alpha[_qp]));
    }
    else
    {
      return 0.0;
