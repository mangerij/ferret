...
#include "NormMagHStrongCart.h"

class NormMagHStrongCart;

registerMooseObject("FerretApp", NormMagHStrongCart);

InputParameters NormMagHStrongCart::validParams()
{
...
NormMagHStrongCart::NormMagHStrongCart(const InputParameters & parameters)
  :Kernel(parameters),
...
Real
NormMagHStrongCart::computeQpResidual()
{
  return -(_grad_test[_i][_qp](0)*_mag_x[_qp]+_grad_test[_i][_qp](1)*_mag_y[_qp]+_grad_test[_i][_qp](2)*_mag_z[_qp]);
}
Real
NormMagHStrongCart::computeQpJacobian()
{
  return 0.0;
}

Real
NormMagHStrongCart::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (jvar == _mag_x_var)
  {
  ...
  {
    return -(_grad_test[_i][_qp](1)*_phi[_j][_qp]);
  }
  else if (jvar == _mag_z_var)
  {
    return -(_grad_test[_i][_qp](2)*_phi[_j][_qp]);
  }
  else
    return 0.0;
}
