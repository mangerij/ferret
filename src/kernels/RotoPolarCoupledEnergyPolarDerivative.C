...
RotoPolarCoupledEnergyPolarDerivative::computeQpResidual()
{
  ...
  else if (_component == 1)
  {
   ...
   (_antiferrodis_A_x[_qp]*_antiferrodis_A_y[_qp]*std::pow(_antiferrodis_A_z[_qp],2)*_polar_x[_qp] + std::pow(_antiferrodis_A_x[_qp],2)*_antiferrodis_A_y[_qp]*_antiferrodis_A_z[_qp]*_polar_z[_qp])*_t24121233 + 2*(std::pow(_antiferrodis_A_x[_qp],6) + std::pow(_antiferrodis_A_z[_qp],6))*_polar_y[_qp]*_t2611222222 + 4*std::pow(_antiferrodis_A_y[_qp],2)*std::pow(_polar_y[_qp],3)*_t42111111 + 4*(std::pow(_antiferrodis_A_x[_qp],2) + std::pow(_antiferrodis_A_z[_qp],2))*std::pow(_polar_y[_qp],3)*_t42111122 + 
   (_antiferrodis_A_x[_qp]*_antiferrodis_A_y[_qp]*std::pow(_polar_x[_qp],3) + _antiferrodis_A_y[_qp]*_antiferrodis_A_z[_qp]*std::pow(_polar_z[_qp],3) + 3*std::pow(_polar_y[_qp],2)*(_antiferrodis_A_x[_qp]*_antiferrodis_A_y[_qp]*_polar_x[_qp] + _antiferrodis_A_y[_qp]*_antiferrodis_A_z[_qp]*_polar_z[_qp]))*_t42111212 + 
...
RotoPolarCoupledEnergyPolarDerivative::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (_component == 0)
  {
   ...
   (_antiferrodis_A_z[_qp]*std::pow(_polar_y[_qp],2)*_polar_z[_qp] + _antiferrodis_A_y[_qp]*_polar_y[_qp]*std::pow(_polar_z[_qp],2))*_t42123312 + 16*std::pow(_antiferrodis_A_x[_qp],3)*std::pow(_polar_x[_qp],3)*_t4411111111 + 12*_antiferrodis_A_x[_qp]*std::pow(_polar_x[_qp],5)*_t6211111111);
    }
  ...
  else if (_component == 1)
  {
   ...
   (_antiferrodis_A_z[_qp]*std::pow(_polar_x[_qp],2)*_polar_z[_qp] + _antiferrodis_A_x[_qp]*_polar_x[_qp]*std::pow(_polar_z[_qp],2))*_t42123312 + 16*std::pow(_antiferrodis_A_y[_qp],3)*std::pow(_polar_y[_qp],3)*_t4411111111 + 12*_antiferrodis_A_y[_qp]*std::pow(_polar_y[_qp],5)*_t6211111111);
    }
    else if (jvar == _antiferrodis_A_z_var)
    {
  ...
  else if (_component == 2)
  {
   ...
   (_antiferrodis_A_y[_qp]*std::pow(_polar_x[_qp],2)*_polar_y[_qp] + _antiferrodis_A_x[_qp]*_polar_x[_qp]*std::pow(_polar_y[_qp],2))*_t42123312 + 16*std::pow(_antiferrodis_A_z[_qp],3)*std::pow(_polar_z[_qp],3)*_t4411111111 + 12*_antiferrodis_A_z[_qp]*std::pow(_polar_z[_qp],5)*_t6211111111);
    }
    else
    {
      return 0.0;
