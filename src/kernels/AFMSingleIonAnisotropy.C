...
#include "AFMSingleIonAnisotropy.h"
#include "libmesh/utility.h"

registerMooseObject("FerretApp", AFMSingleIonAnisotropy);

InputParameters AFMSingleIonAnisotropy::validParams()
{
...
AFMSingleIonAnisotropy::AFMSingleIonAnisotropy(const InputParameters & parameters)
  :Kernel(parameters),
...
Real
AFMSingleIonAnisotropy::computeQpResidual()
{
  if (_mag_sub == 0)
  {
    if (_component == 0)
...
Real
AFMSingleIonAnisotropy::computeQpJacobian()
{
  if (_mag_sub == 0)
  {
    if (_component == 0)
...
Real
AFMSingleIonAnisotropy::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (_mag_sub == 0)
  {
    if (_component == 0)
