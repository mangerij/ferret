...
#include "InducedPolarElectricEStrong.h"
...
InducedPolarElectricEStrong::InducedPolarElectricEStrong(const InputParameters & parameters)
  :Kernel(parameters),
...
Real
InducedPolarElectricEStrong::computeQpResidual()
{
  Real RpolarE = 0.0;
  RpolarE += - ((_polar_x[_qp] +_induced_polar_x[_qp])* _grad_test[_i][_qp](0) + (_polar_y[_qp] +_induced_polar_y[_qp]) * _grad_test[_i][_qp](1) + (_polar_z[_qp] +_induced_polar_z[_qp])* _grad_test[_i][_qp](2)) * std::pow(_len_scale, 2.0);
  ///  Moose::out << "\n R_polarE-"; std::cout << " = " << RpolarE;
  return RpolarE;
}
Real
InducedPolarElectricEStrong::computeQpJacobian()
{
  return 0.0;
}

Real
InducedPolarElectricEStrong::computeQpOffDiagJacobian(unsigned int jvar)
{
