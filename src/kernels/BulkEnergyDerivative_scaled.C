/**
 * @file   BulkEnergyDerivative_scaled.C
...
//Constructor
BulkEnergyDerivative_scaled::BulkEnergyDerivative_scaled(const std::string & name, InputParameters parameters)
...
Real
BulkEnergyDerivative_scaled::computeQpResidual()
{
  ...
  return pow(_len_scale,3.0)*_alpha1*(2*_polar_i[_qp]+
  4*_alpha11*(1/_alpha1)*pow(_polar_i[_qp],3)+
  2*_alpha12*(1/_alpha1)*_polar_i[_qp]*(pow(_polar_j[_qp],2)+pow(_polar_k[_qp],2))+
  6*_alpha111*(1/_alpha1)*pow(_polar_i[_qp],5)+
  4*_alpha112*(1/_alpha1)*pow(_polar_i[_qp],3)*(_polar_j[_qp]*_polar_j[_qp]+_polar_k[_qp]*_polar_k[_qp])+
  2*_alpha112*(1/_alpha1)*_polar_i[_qp]*(pow(_polar_j[_qp],4)+pow(_polar_k[_qp],4))+
  2*_alpha123*(1/_alpha1)*_polar_i[_qp]*pow(_polar_j[_qp],2)*pow(_polar_k[_qp],2))*_test[_i][_qp];
}

Real
BulkEnergyDerivative_scaled::computeQpJacobian()
{
  const VariableValue& _polar_i= (_component==0)? _polar_x : (_component==1)? _polar_y: _polar_z;
  const VariableValue& _polar_j= (_component==0)? _polar_y : (_component==1)? _polar_z: _polar_x;
  const VariableValue& _polar_k= (_component==0)? _polar_z : (_component==1)? _polar_x: _polar_y;
   return pow(_len_scale,3.0)*_alpha1*(
  2+
  12*_alpha11*(1/_alpha1)*pow(_polar_i[_qp],2)+
  2*_alpha12*(1/_alpha1)*(pow(_polar_j[_qp],2)+pow(_polar_k[_qp],2))+
  30*_alpha111*(1/_alpha1)*pow(_polar_i[_qp],4)+
  12*_alpha112*(1/_alpha1)*pow(_polar_i[_qp],2)*(pow(_polar_j[_qp],2)+pow(_polar_k[_qp],2))+
  2*_alpha112*(1/_alpha1)*(pow(_polar_j[_qp],4)+pow(_polar_k[_qp],4))+
  2*_alpha123*(1/_alpha1)*pow(_polar_j[_qp],2)*pow(_polar_k[_qp],2)
	  )*_test[_i][_qp]*_phi[_j][_qp];
}

Real
BulkEnergyDerivative_scaled::computeQpOffDiagJacobian(unsigned int jvar)
{
  Real r;
  ...
  if(jvar==_polar_x_var || jvar==_polar_y_var || jvar==_polar_z_var){
    ...
    r=(4*_alpha12*(1/_alpha1)*_polar_i[_qp]*_polar_j[_qp]+
           8*_alpha112*(1/_alpha1)*pow(_polar_i[_qp],3)*_polar_j[_qp]+
           8*_alpha112*(1/_alpha1)*_polar_i[_qp]*pow(_polar_j[_qp],3)+
           4*_alpha123*(1/_alpha1)*_polar_i[_qp]*_polar_j[_qp]*pow(_polar_k[_qp],2));
    return r*_test[_i][_qp]*_phi[_j][_qp]*pow(_len_scale,3.0)*_alpha1;
  }else
    return 0.0;
}
