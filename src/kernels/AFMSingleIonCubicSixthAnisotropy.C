...
registerMooseObject("FerretApp", AFMSingleIonCubicSixthAnisotropy);

InputParameters AFMSingleIonCubicSixthAnisotropy::validParams()
{
...
AFMSingleIonCubicSixthAnisotropy::AFMSingleIonCubicSixthAnisotropy(const InputParameters & parameters)
  :Kernel(parameters),
...
Real
AFMSingleIonCubicSixthAnisotropy::computeQpResidual()
{
  if (_mag_sub == 0)
  {
    if (_component == 0)
...
Real
AFMSingleIonCubicSixthAnisotropy::computeQpJacobian()
{
  if (_mag_sub == 0)
  {
    if (_component == 0)
...
Real
AFMSingleIonCubicSixthAnisotropy::computeQpOffDiagJacobian(unsigned int jvar)
{
  if (_mag_sub == 0)
  {
    if (_component == 0)
    ...
    else if (_component == 1)
    {
      if (jvar == _mag1_x_var)
      {
        return (2.0*_g0mu0Ms[_qp]*(_K1c[_qp] + (Utility::pow<2>(_antiphase_A_x[_qp]) + Utility::pow<2>(_antiphase_A_y[_qp]) + Utility::pow<2>(_antiphase_A_z[_qp]))*_Kt[_qp])*_mag1_y[_qp]*_mag1_z[_qp]*(-3.0*Utility::pow<2>(_mag1_x[_qp])*_mag1_y[_qp] - 4*_alpha[_qp]*Utility::pow<3>(_mag1_x[_qp])*_mag1_z[_qp] + _mag1_y[_qp]*Utility::pow<2>(_mag1_z[_qp]) + _alpha[_qp]*_mag1_x[_qp]*(4*Utility::pow<2>(_mag1_y[_qp])*_mag1_z[_qp] - 2.0*Utility::pow<3>(_mag1_z[_qp])))*_phi[_j][_qp]*_test[_i][_qp])/
   (1.0 + Utility::pow<2>(_alpha[_qp]));
      }
      else if (jvar == _mag1_z_var)
      {
        return (-2.0*_g0mu0Ms[_qp]*(_K1c[_qp] + (Utility::pow<2>(_antiphase_A_x[_qp]) + Utility::pow<2>(_antiphase_A_y[_qp]) + Utility::pow<2>(_antiphase_A_z[_qp]))*_Kt[_qp])*_mag1_x[_qp]*_mag1_y[_qp]*(Utility::pow<2>(_mag1_x[_qp])*_mag1_y[_qp] + 2.0*_alpha[_qp]*Utility::pow<3>(_mag1_x[_qp])*_mag1_z[_qp] - 3.0*_mag1_y[_qp]*Utility::pow<2>(_mag1_z[_qp]) + 4*_alpha[_qp]*_mag1_x[_qp]*_mag1_z[_qp]*(-Utility::pow<2>(_mag1_y[_qp]) + Utility::pow<2>(_mag1_z[_qp])))*_phi[_j][_qp]*_test[_i][_qp])/(1.0 + Utility::pow<2>(_alpha[_qp]));
      }
      else if (jvar == _mag2_x_var)
      {
        return 0.0;
  ...
  else if (_mag_sub == 1)
  {
    if (_component == 0)
    {
      if (jvar == _mag2_y_var)
      {
        return (2.0*_g0mu0Ms[_qp]*(_K1c[_qp] + (Utility::pow<2>(_antiphase_A_x[_qp]) + Utility::pow<2>(_antiphase_A_y[_qp]) + Utility::pow<2>(_antiphase_A_z[_qp]))*_Kt[_qp])*_mag2_x[_qp]*_mag2_z[_qp]*(4*_alpha[_qp]*Utility::pow<2>(_mag2_x[_qp])*_mag2_y[_qp]*_mag2_z[_qp] + _mag2_x[_qp]*(3.0*Utility::pow<2>(_mag2_y[_qp]) - Utility::pow<2>(_mag2_z[_qp])) - 2.0*_alpha[_qp]*_mag2_y[_qp]*_mag2_z[_qp]*(2.0*Utility::pow<2>(_mag2_y[_qp]) + Utility::pow<2>(_mag2_z[_qp])))*_phi[_j][_qp]*_test[_i][_qp])/
   (1.0 + Utility::pow<2>(_alpha[_qp]));
      }
      else if (jvar == _mag2_z_var)
      {
        return (2.0*_g0mu0Ms[_qp]*(_K1c[_qp] + (Utility::pow<2>(_antiphase_A_x[_qp]) + Utility::pow<2>(_antiphase_A_y[_qp]) + Utility::pow<2>(_antiphase_A_z[_qp]))*_Kt[_qp])*_mag2_x[_qp]*_mag2_y[_qp]*(4*_alpha[_qp]*Utility::pow<2>(_mag2_x[_qp])*_mag2_y[_qp]*_mag2_z[_qp] + _mag2_x[_qp]*(Utility::pow<2>(_mag2_y[_qp]) - 3.0*Utility::pow<2>(_mag2_z[_qp])) - 2.0*_alpha[_qp]*_mag2_z[_qp]*(Utility::pow<3>(_mag2_y[_qp]) + 2.0*_mag2_y[_qp]*Utility::pow<2>(_mag2_z[_qp])))*_phi[_j][_qp]*_test[_i][_qp])/
   (1.0 + Utility::pow<2>(_alpha[_qp]));
      }
    ...
    else if (_component == 1)
    {
      if (jvar == _mag2_x_var)
      {
        return (2.0*_g0mu0Ms[_qp]*(_K1c[_qp] + (Utility::pow<2>(_antiphase_A_x[_qp]) + Utility::pow<2>(_antiphase_A_y[_qp]) + Utility::pow<2>(_antiphase_A_z[_qp]))*_Kt[_qp])*_mag2_y[_qp]*_mag2_z[_qp]*(-3.0*Utility::pow<2>(_mag2_x[_qp])*_mag2_y[_qp] - 4*_alpha[_qp]*Utility::pow<3>(_mag2_x[_qp])*_mag2_z[_qp] + _mag2_y[_qp]*Utility::pow<2>(_mag2_z[_qp]) + _alpha[_qp]*_mag2_x[_qp]*(4*Utility::pow<2>(_mag2_y[_qp])*_mag2_z[_qp] - 2.0*Utility::pow<3>(_mag2_z[_qp])))*_phi[_j][_qp]*_test[_i][_qp])/
   (1.0 + Utility::pow<2>(_alpha[_qp]));
      }
      else if (jvar == _mag2_z_var)
      {
        return (-2.0*_g0mu0Ms[_qp]*(_K1c[_qp] + (Utility::pow<2>(_antiphase_A_x[_qp]) + Utility::pow<2>(_antiphase_A_y[_qp]) + Utility::pow<2>(_antiphase_A_z[_qp]))*_Kt[_qp])*_mag2_x[_qp]*_mag2_y[_qp]*(Utility::pow<2>(_mag2_x[_qp])*_mag2_y[_qp] + 2.0*_alpha[_qp]*Utility::pow<3>(_mag2_x[_qp])*_mag2_z[_qp] - 3.0*_mag2_y[_qp]*Utility::pow<2>(_mag2_z[_qp]) + 4*_alpha[_qp]*_mag2_x[_qp]*_mag2_z[_qp]*(-Utility::pow<2>(_mag2_y[_qp]) + Utility::pow<2>(_mag2_z[_qp])))*_phi[_j][_qp]*_test[_i][_qp])/(1.0 + Utility::pow<2>(_alpha[_qp]));
      }
      else if (jvar == _mag1_x_var)
      {
        return 0.0;
