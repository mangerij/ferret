...
#include "InteractionUSLLGAlt.h"
#include "libmesh/utility.h"

registerMooseObject("FerretApp", InteractionUSLLGAlt);

template<>
InputParameters validParams<InteractionUSLLGAlt>()
{
...
InteractionUSLLGAlt::InteractionUSLLGAlt(const InputParameters & parameters)
  :Kernel(parameters),
...
Real
InteractionUSLLGAlt::computeQpJacobian()
{
  if (_component == 0)
  {
    return (_alpha*_g0*_phi[_j][_qp]*_test[_i][_qp]*(_potential_H_int_grad[_qp](0)*std::cos(_azimuth_phi[_qp]) + _potential_H_int_grad[_qp](2)*(std::cos(_polar_theta[_qp])/std::sin(_polar_theta[_qp])) + _potential_H_int_grad[_qp](1)*std::sin(_azimuth_phi[_qp]))*std::sqrt(Utility::pow<2>(std::sin(_polar_theta[_qp]))))/(1 + Utility::pow<2>(_alpha));
  }
  else if (_component == 1)
  {
    return (_g0*_phi[_j][_qp]*_test[_i][_qp]*(std::cos(_azimuth_phi[_qp])*(_alpha*_potential_H_int_grad[_qp](0) + _potential_H_int_grad[_qp](1)*std::cos(_polar_theta[_qp]))*(1.0/std::sin(_polar_theta[_qp])) + (-(_potential_H_int_grad[_qp](0)*(std::cos(_polar_theta[_qp])/std::sin(_polar_theta[_qp]))) + _alpha*_potential_H_int_grad[_qp](1)*(1.0/std::sin(_polar_theta[_qp])))*std::sin(_azimuth_phi[_qp])))/(1 + Utility::pow<2>(_alpha));
  }
  else
    return 0.0;
}
