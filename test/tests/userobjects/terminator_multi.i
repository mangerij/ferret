...
p0x = 0.0
p0y = 0.0
p0z = 0.290051              # converged c-phase value from README.md
noise = 1e-6                # RandomIC needs min < max; this is effectively no spread

ftol = 5.0e-3               # fires on step 3 (perc_change: 6.8e-3, 4.4e-3, ...)
tmin = 0.5                  # ... but not before Ftotal is populated
...
[MultiApps]
  [polar]
    ...
    input_files = terminator_multi_polar.i
  ...
  [mech]
    type = TransientMultiApp
    input_files = terminator_multi_mech.i
    cli_args = '${args}'
    execute_on = TIMESTEP_END
...
[Executioner]
  ...
  num_steps = 6               # BACKSTOP ONLY -- the Terminator should stop this at step 3
[]
